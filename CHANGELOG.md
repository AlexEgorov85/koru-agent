...
### Fixed
- Исправлены компоненты и провайдеры LLM
- Исправлена конфигурация и типы векторов
- Обновлены интеграционные тесты
- Исправлен владелец прав в LICENSE (заменено "koru-agent Team" на "Egorov A.V.")

### Changed
- Обновлена версия проекта до 5.46.2
...
### Changed
- **observation_formatter: улучшенное форматирование SQL-наблюдений**
  - Убран SQL запрос из начала наблюдения (он был артефактом генерации)
  - Добавлен формат: "Выполнен запрос {SQL}; получены данные {результат}"
  - Малые данные (< 5 строк и < 500 символов) отображаются как Markdown таблица
  - Большие данные показывают статистику с рекомендацией использовать data_analysis
...
### Fixed
- **generate_script_handler: Добавлен цикл валидации и корректировки SQL**
  - До 3 попыток генерации SQL с передачей ошибки в LLM
  - `_validate_sql()`: проверка синтаксиса, баланса скобок, опасных операций
  - `_execute_sql()`: бросает `SQLGenerationError` при ошибке (для срабатывания retry)
  - LLM получает полный текст ошибки: "Предыдущая ошибка: ...\nИсправь SQL запрос."

- **base_handler: Добавлены описания колонок в схему БД**
  - `_get_default_schema_fallback()` теперь включает `col_description` для каждой колонки
  - Добавлен заголовок таблицы с полным описанием
  - Размер схемы: ~4KB (4 таблицы, 41 колонка с описаниями)
  - Без описаний LLM не мог корректно формировать запросы

- **vllm_provider: Исправлена ошибка подсчета токенов**
  - `promt_token_ids` → `outputs[0].token_ids`
  - Считает токены ответа а не промпта
