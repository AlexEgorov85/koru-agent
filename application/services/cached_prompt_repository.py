...
class CachedPromptRepository(IPromptRepository):
    ...
    def get_active_version(
        ...
        version = self._underlying_repo.get_active_version(domain, capability_name, provider_type, role)
    ...
    def get_shadow_version(
        ...
        # Запрашиваем из underlying repo
        version = self._underlying_repo.get_shadow_version(domain, capability_name, provider_type, role)
    ...
    def get_version_by_id(self, version_id: str) -> Optional[PromptVersion]:
        ...
        # Запрашиваем из underlying repo
        version = self._underlying_repo.get_version_by_id(version_id)
    ...
    def save_version(self, version: PromptVersion) -> None:
        ...
        self._underlying_repo.save_version(version)
    ...
    def update_version_status(self, version_id: str, status: PromptStatus) -> None:
        ...
        self._underlying_repo.update_version_status(version_id, status)
    ...
    def activate_version(self, version_id: str) -> None:
        ...
        self._underlying_repo.activate_version(version_id)
        
        # Обновляем кэш - получаем напрямую из underlying repo, чтобы получить актуальный статус
        new_version = self._underlying_repo.get_version_by_id(version_id)
        if new_version:
            # Обновляем кэш
            with self._lock:
    ...
    def archive_version(self, version_id: str) -> None:
        ...
        self._underlying_repo.archive_version(version_id)
    ...
    def list_versions(self, capability_name: str) -> List[PromptVersion]:
        ...
        if len(result) != len(cached_ids):
            db_versions = self._underlying_repo.list_versions(capability_name)
            result = db_versions
    ...
    def list_versions_by_address(
        ...
        # Если в кэше нет всех версий, запрашиваем из underlying repo
        if len(result) == 0:
            db_versions = self._underlying_repo.list_versions_by_address(
    ...
    def update_usage_metrics(
        ...
        self._underlying_repo.update_usage_metrics(version_id, metrics_update)
