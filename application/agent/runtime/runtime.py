...
from domain.abstractions.event_system import EventType, IEventPublisher
...
class AgentRuntime:
    ...
    async def initialize(self) -> bool:
        ...
        available = self.skill_registry.get_all_skills()
        if not available:
            await self.event_publisher.publish(
                event_type=EventType.WARNING,
                source="AgentRuntime",
                data={"message": "Нет доступных навыков для выполнения задачи"}
            )
            return False
