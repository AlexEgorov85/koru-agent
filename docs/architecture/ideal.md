...
|---------|----------|--------|
| **🌱 Начальный** | Есть разделение `infrastructure/` и `application/` | ✅ Достигнут |
| **🌿 Развитый** | Изолированные кэши + валидация статусов | ⚠️ Частично (сервисы регистрируются в инфраструктуре) |
| **🌳 Зрелый** | Горячее переключение + профили prod/sandbox | ❌ Не достигнут |
| **🌲 Продакшн-готовый** | Все критерии из чек-листа + стресс-тесты | ❌ Не достигнут |
| **🌲🌲 Идеальный** | + Чистая конфигурация без дублирования | ❌ Не достигнут |

**Текущий уровень:** 🌿 Развитый (65% готовности)  
**Главный барьер:** Дублирование конфигурации (`agent_config` в `dev.yaml`)

---

## 🚀 План достижения идеальной архитектуры

| Этап | Задача | Срок | Критерий завершения |
|------|--------|------|---------------------|
| **1** | Удалить `agent_config` и `prompt_versions` из `dev.yaml` | 1 день | `grep -r "agent_config\|prompt_versions" core/config/defaults/` → 0 совпадений |
| **2** | Перенести версии в `data/registry.yaml` | 1 день | Все версии загружаются из `registry.yaml`, а не из `dev.yaml` |
| **3** | Перенос создания `PromptService`/`ContractService` из `InfrastructureContext` в `ApplicationContext` | 2 дня | Тест изоляции кэшей проходит ✅ |
| **4** | Исправление конфигурации: `type_provider` → `provider_type` | 1 день | Все провайдеры регистрируются ✅ |
| **5** | Реализация профилей `prod`/`sandbox` с валидацией статусов | 2 дня | Продакшн отклоняет `draft` версии ✅ |
| **6** | Горячее переключение версий через `clone_with_version_override()` | 1 день | Переключение < 50 мс ✅ |
| **7** | Стресс-тест: 50 параллельных агентов | 1 день | Память < 1.2 ГБ, все завершаются успешно ✅ |

**Итого:** 9 рабочих дней до продакшн-готовой архитектуры без дублирования конфигурации.
