...
### Новые возможности (Апрель 2026)

| Задача | Статус | Файлы | Описание |
|--------|--------|--------|----------|
| RowMetadata | ✅ | `vector_types.py`, `indexer.py` | Универсальная структура для хранения строк БД |
| top_k=None | ✅ | `faiss_provider.py`, `search_input` | Безлимитный поиск по min_score |
| VectorSearchDefaults | ✅ | `vector_search_tool.py` | Константы для типичных сценариев |
...
### Хочу использовать безлимитный поиск
→ `UNIVERSAL_SPEC.md` → раздел «Управление top_k»

### Хочу получить все поля строки из результата
→ `UNIVERSAL_SPEC.md` → `RowMetadata`
