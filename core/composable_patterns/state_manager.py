...
class ComposablePatternStateManager:
    ...
    def create_state(
        ...
        self.logger.info(f"Создано состояние для паттерна '{pattern_name}' с ID: {state_id}")
        return state
    ...
    def update_state(self, state_id: str, updates: Dict[str, Any]) -> bool:
        ...
        if state_id not in self.pattern_states:
            self.logger.warning(f"Состояние с ID '{state_id}' не найдено для обновления")
            return False
        ...
        self.logger.debug(f"Обновлено состояние с ID '{state_id}'")
        return True
    ...
    def start_action_execution(self, state_id: str, action_name: str) -> bool:
        ...
        """
        if state_id not in self.pattern_states:
            self.logger.warning(f"Состояние с ID '{state_id}' не найдено")
            return False
        
        state = self.pattern_states[state_id]
        state.start_action_execution(action_name)
        
        self.logger.info(f"Начало выполнения действия '{action_name}' в паттерне '{state.pattern_name}' (ID: {state_id})")
        return True
    ...
    def finish_action_execution(self, state_id: str, action_result: Dict[str, Any]) -> bool:
        ...
        self.logger.info(f"Завершено действие в паттерне '{state.pattern_name}', всего выполнено действий: {state.action_index}")
        return True
    ...
    def register_error(self, state_id: str) -> bool:
        ...
        self.logger.error(f"Зарегистрирована ошибка в паттерне '{state.pattern_name}' (ID: {state_id})")
        return True
    ...
    def register_progress(self, state_id: str, progressed: bool) -> bool:
        ...
        self.logger.info(f"Зарегистрирован {progress_status} в паттерне '{state.pattern_name}' (ID: {state_id})")
        return True
    ...
    def complete(self, state_id: str) -> bool:
        ...
        self.logger.info(f"Паттерн '{state.pattern_name}' помечен как завершенный (ID: {state_id})")
        return True
    ...
    def pause(self, state_id: str) -> bool:
        ...
        self.logger.info(f"Паттерн '{state.pattern_name}' приостановлен (ID: {state_id})")
        return True
    ...
    def resume(self, state_id: str) -> bool:
        ...
        self.logger.info(f"Паттерн '{state.pattern_name}' возобновлен (ID: {state_id})")
        return True
    ...
    def waiting_for_input(self, state_id: str) -> bool:
        ...
        self.logger.info(f"Паттерн '{state.pattern_name}' ожидает ввода (ID: {state_id})")
        return True
    ...
    def remove_state(self, state_id: str) -> bool:
        ...
        if state_id in self.pattern_states:
            ...
            self.logger.info(f"Удалено состояние с ID '{state_id}'")
            return True
        else:
            self.logger.warning(f"Состояние с ID '{state_id}' не найдено для удаления")
            return False