"""
Фабрика для создания ресурсов из конфигурации.
ОСОБЕННОСТИ:
...
class ProviderFactory:
                ...
                try:
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        ...
                        if issubclass(obj, BaseTool) and obj != BaseTool:
                            ...
                            tool_classes[obj.name] = obj
                            logger.debug(f"Найден класс инструмента '{obj.name}'")
                            ...
                            try:
                                ...
                                for name, obj in inspect.getmembers(module, inspect.isclass):
                                    ...
                                    if hasattr(obj, '__bases__') and any('BaseSkill' in str(base) for base in obj.__bases__):
                                        skill_classes[obj.name] = obj
                                        logger.debug(f"Найден класс навыка '{obj.name}'")
    ...
    async def discover_and_create_all_services(self) -> Dict[str, BaseService]:
        """
        Автоматическое обнаружение и создание ВСЕХ сервисов из директории.
        
        АРХИТЕКТУРА ЖИЗНЕННОГО ЦИКЛА:
        1. Создание экземпляров (без инициализации) — все сервисы доступны для регистрации
        2. Регистрация в реестре — все сервисы видны друг другу
        3. Инициализация — сервисы могут безопасно получать зависимости из реестра
        
        Возвращает:
            Словарь {имя_сервиса: экземпляр_сервиса}
        """
        services = {}

        # 0. Регистрация базовых сервисов (языковой реестр)
        await self._register_base_services()
        
        # 1. Обнаружение классов сервисов
        discovered_services = await self._discover_services_from_directory()
        
        # 2. Создание экземпляров БЕЗ инициализации
        #    (чтобы избежать проблем с порядком зависимостей)
        for service_name, service_class in discovered_services.items():
            ...
            # Создаём экземпляр БЕЗ вызова initialize()
            service = await self._create_service_instance_without_init(
                service_class, service_name, service_config
            )
            if service:
                services[service_name] = service
        
        if not services:
            logger.warning("Не найдено ни одного активного сервиса для регистрации")
            return {}
        
        # 3. РЕГИСТРАЦИЯ всех сервисов в реестре
        for service_name, service in services.items():
            ...
            resource_info.is_default = False
            self.system_context.registry.register(resource_info)
            logger.info(f"Сервис '{service_name}' зарегистрирован в реестре")
        
        return services

    async def _create_service_instance_without_init(
        self,
        ...
        """
        Создание экземпляра сервиса БЕЗ вызова метода initialize().
        
        ВАЖНО: Инициализация выполняется отдельно после регистрации всех сервисов
        для корректного разрешения зависимостей.
        """
        try:
            logger.debug(f"Создание экземпляра сервиса '{service_name}' из класса {service_class.__name__}")
            ...
            # Создание экземпляра БЕЗ инициализации
            service = service_class(**init_params)
            
            # Помечаем как неинициализированный (для отладки)
            if hasattr(service, "_initialized"):
                service._initialized = False
            
            logger.debug(f"Экземпляр сервиса '{service_name}' создан")
            return service
        
        except Exception as e:
            logger.error(f"Ошибка создания экземпляра сервиса '{service_name}': {str(e)}", exc_info=True)
            return None
    
    async def _discover_services_from_directory(self) -> Dict[str, Type[BaseService]]:
        """Обнаружение всех классов сервисов в директории сервисов.
        
        Возвращает:
            Словарь {имя_сервиса: класс_сервиса} для всех найденных сервисов
        """
        service_classes = {}
        ...
        try:
            ...
            for root, dirs, files in os.walk(services_dir):
                for dir_name in dirs:
                    if dir_name.startswith("__") or dir_name in ["base", "templates", "adapters"]:
                        continue
                    
                    service_subdir = Path(root) / dir_name
                    service_package_path = str(service_subdir).replace("/", ".").replace("\\", ".")
                    
                    try:
                        # Импортируем подпакет сервиса
                        service_package = importlib.import_module(service_package_path)
                        ...
                        for _, module_name, _ in pkgutil.iter_modules(service_package.__path__):
                            ...
                            full_module_name = f"{service_package_path}.{module_name}"
                            try:
                                # Динамически импортируем модуль
                                module = importlib.import_module(full_module_name)
                                ...
                                for name, obj in inspect.getmembers(module, inspect.isclass):
                                    ...
                                    if issubclass(obj, BaseService) and obj != BaseService:
                                        ...
                                        service_classes[obj.name] = obj
                                        logger.debug(f"Найден класс сервиса '{name}' -> зарегистрирован как '{obj.name}'")
                            
                            except Exception as e:
                                logger.error(f"Ошибка при загрузке модуля '{full_module_name}': {str(e)}")
                    
                    except Exception as e:
                        logger.error(f"Ошибка при импорте пакета сервисов '{service_package_path}': {str(e)}")
    ...
    def _get_service_config(self, service_name: str) -> Dict[str, Any]:
        ...
        Возвращает:
            Словарь с конфигурацией сервиса или конфигурацию по умолчанию
        """
    ...
    async def _create_service_instance(
        ...
        Возвращает:
            Экземпляр сервиса или None при ошибке
        """
        try:
            ...
            if hasattr(service, "initialize") and callable(service.initialize):
                ...
                if not success:
                    logger.error(f"Не удалось инициализировать сервис '{service_name}'")
                    return None
            
            logger.info(f"Сервис '{service_name}' успешно создан и инициализирован")
            return service
    ...
    async def _register_base_services(self) -> None:
        """Регистрация базовых сервисов, необходимых для работы других сервисов."""
        # 1. Создаём и регистрируем реестр языков
        from core.infrastructure.services.code_analysis.language_registry import LanguageRegistry
        from core.infrastructure.services.code_analysis.adapters.python_adapter import PythonLanguageAdapter
        
        language_registry = LanguageRegistry()
        language_registry.register_adapter(PythonLanguageAdapter())
        
        # Регистрируем как сервис ДО обнаружения остальных сервисов
        self.system_context.registry.register(ResourceInfo(
            name="language_registry",
            resource_type=ResourceType.SERVICE,
            instance=language_registry
        ))
        
        logger.info("Базовые сервисы (language_registry) зарегистрированы")