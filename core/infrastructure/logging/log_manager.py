...
ВАЖНО: Этот модуль НЕ использует logging для избежания циклической зависимости.
"""
import os
import sys
import asyncio
...
def _debug(msg: str) -> None:
    ...
    if os.environ.get("KORU_DEBUG"):
        print(f"[LogManager DEBUG] {msg}", file=sys.stderr)


def _info(msg: str) -> None:
    """Информационный вывод (не через logging)."""
    print(f"[LogManager] {msg}", file=sys.stderr)


def _error(msg: str) -> None:
    """Вывод ошибок (не через logging)."""
    print(f"[LogManager ERROR] {msg}", file=sys.stderr)
...
class LogManager:
    ...
    async def initialize(self) -> None:
        ...
        if self._initialized:
            _debug("LogManager уже инициализирован")
            return

        _info("Инициализация LogManager...")
        ...
        for dir_path in dirs_to_create + archive_dirs:
            ...
            _debug(f"Создана директория: {dir_path}")
        ...
        _info("LogManager успешно инициализирован")
    ...
    async def _create_symlinks(self) -> None:
        ...
        try:
            ...
            if agent_file.exists():
                ...
                _debug(f"Создан symlink: {agent_symlink} → {agent_file}")
            ...
            _error(f"Ошибка создания symlink: {e}")
    ...
    def log_agent(self, message: str, level: str = "INFO", **kwargs) -> None:
        ...
        if not self._initialized:
            _debug("LogManager не инициализирован, пропуск лога")
            return
        ...
        try:
            ...
            _error(f"Ошибка записи лога агента: {e}")
    ...
    def log_session(self, session_id: str, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            _error(f"Ошибка записи лога сессии: {e}")
    ...
    def _update_session_symlink(self, session_id: str, log_path: Path) -> None:
        ...
        try:
            ...
            _debug(f"Ошибка обновления symlink сессии: {e}")
    ...
    def log_llm(self, session_id: str, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            _error(f"Ошибка записи LLM лога: {e}")
    ...
    def _update_llm_symlink(self, log_path: Path) -> None:
        ...
        try:
            ...
            _debug(f"Ошибка обновления symlink LLM: {e}")
    ...
    def log_metrics(self, capability: str, metrics_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            _error(f"Ошибка записи метрик: {e}")
    ...
    def _save_config(self) -> None:
        ...
        try:
            ...
            _debug(f"Конфигурация сохранена: {config_path}")
        except Exception as e:
            _error(f"Ошибка сохранения конфигурации: {e}")
    ...
    async def shutdown(self) -> None:
        ...
        _info("Завершение работы LogManager...")
        ...
        self._initialized = False
        _info("LogManager завершил работу")
