...
class ProviderLifecycleManager:
    ...
    def __init__(self, event_bus=None):
        ...
        if event_bus is not None:
            self.event_bus_logger = EventBusLogger(event_bus, session_id="system", agent_id="system", component="ProviderLifecycleManager")
            self._use_event_logging = True
        else:
            import logging
            self.event_bus_logger = logging.getLogger(f"{__name__}.LifecycleManager")
            self._use_event_logging = False
        
        self._log_info("ProviderLifecycleManager создан")

    def _log_info(self, message: str, *args, **kwargs):
        """Информационное сообщение."""
        if self._use_event_logging:
            self.event_bus_logger.info_sync(message, *args, **kwargs)
        else:
            self.event_bus_logger.info(message, *args, **kwargs)

    def _log_debug(self, message: str, *args, **kwargs):
        """Отладочное сообщение."""
        if self._use_event_logging:
            self.event_bus_logger.debug_sync(message, *args, **kwargs)
        else:
            self.event_bus_logger.debug(message, *args, **kwargs)

    def _log_warning(self, message: str, *args, **kwargs):
        """Предупреждение."""
        if self._use_event_logging:
            self.event_bus_logger.warning_sync(message, *args, **kwargs)
        else:
            self.event_bus_logger.warning(message, *args, **kwargs)

    def _log_error(self, message: str, *args, **kwargs):
        """Ошибка."""
        if self._use_event_logging:
            self.event_bus_logger.error_sync(message, *args, **kwargs)
        else:
            self.event_bus_logger.error(message, *args, **kwargs)
    ...
    async def register(
        ...
        self._log_info(f"Зарегистрирован провайдер '{name}' типа {provider_type.value}")
    ...
    async def unregister(self, name: str) -> bool:
        ...
        if name not in self._providers:
            self._log_warning(f"Провайдер '{name}' не найден")
            return False
        ...
        self._log_info(f"Удален провайдер '{name}'")
    ...
    async def initialize_all(self) -> Dict[str, bool]:
        ...
        if self._shutdown_in_progress:
            self._log_warning("Инициализация во время shutdown запрещена")
            return {}
        
        results = {}
        
        self._log_info(f"Начало инициализации {len(self._providers)} провайдеров")
        ...
        for provider_type in init_order:
            ...
            self._log_debug(f"Инициализация провайдеров типа {provider_type.value}: {provider_names}")
        ...
        self._log_info(f"Инициализация завершена: {sum(results.values())}/{len(results)} успешно")
    ...
    async def _initialize_provider(self, provider_info: ProviderInfo) -> bool:
        ...
        try:
            self._log_debug(f"Инициализация провайдера '{name}'")
            ...
            if result:
                ...
                self._log_info(f"Провайдер '{name}' успешно инициализирован")
            else:
                provider_info.health_status = ProviderHealthStatus.UNHEALTHY
                self._log_error(f"Провайдер '{name}' вернул False при инициализации")
            
            return result
            
        except Exception as e:
            provider_info.health_status = ProviderHealthStatus.UNHEALTHY
            self._log_error(f"Ошибка инициализации провайдера '{name}': {e}", exc_info=True)
    ...
    async def shutdown_all(self) -> Dict[str, bool]:
        ...
        self._log_info("Начало завершения работы провайдеров")
        ...
        self._log_info(f"Завершение работы провайдеров: {sum(results.values())}/{len(results)} успешно")
    ...
    async def _shutdown_provider(self, provider_info: ProviderInfo) -> bool:
        ...
        if not provider_info.initialized:
            self._log_debug(f"Провайдер '{name}' не инициализирован, пропускаем shutdown")
            return True
        
        try:
            self._log_debug(f"Завершение работы провайдера '{name}'")
            
            await provider.shutdown()
            
            provider_info.initialized = False
            provider_info.health_status = ProviderHealthStatus.UNKNOWN
            self._log_info(f"Провайдер '{name}' корректно завершен")
            
            return True
            
        except Exception as e:
            self._log_error(f"Ошибка при завершении провайдера '{name}': {e}", exc_info=True)
            return False
    ...
    async def _health_check_provider(self, provider_info: ProviderInfo) -> HealthCheckResult:
        ...
        try:
            ...
            provider_info.health_status = ProviderHealthStatus.UNHEALTHY
            self._log_error(f"Health check провайдера '{name}' не удался: {e}")
