...
class AgentFactory:
    ...
    async def _validate_version_consistency(self, config: AgentConfig) -> List[str]:
        ...
        # Проверка промптов
        prompt_service = self.infrastructure.get_service("prompt_service")
        if prompt_service:
            for capability, version in config.prompt_versions.items():
                exists = await prompt_service.check_version_exists(capability, version)
                if not exists:
                    errors.append(f"Промпт {capability}@{version} не существует")

        # Проверка контрактов (для совместимости используем общее поле)
        contract_service = self.infrastructure.get_service("contract_service")
        if contract_service:
            # Проверяем контракты из общего поля
            for capability, version in getattr(config, 'contract_versions', {}).items():
                # Проверяем как input, так и output
                input_exists = await contract_service.check_version_exists(capability, version, "input")
                if not input_exists:
                    errors.append(f"Input-контракт {capability}@{version} не существует")
                
                output_exists = await contract_service.check_version_exists(capability, version, "output")
