...
class CachedBaseService(BaseService):
    """
    Абстрактный базовый класс для всех инфраструктурных сервисов с кэшированием.
    """

    def __init__(self, system_context: BaseSystemContext, name: Optional[str] = None, component_config: Optional[ComponentConfig] = None):
    ...
    async def initialize_with_config(self, agent_config: Optional[AgentConfig] = None) -> bool:
        ...
        if self.component_config:
            ...
            self.logger.info(
                f"Сервис '{self.name}' инициализирован с вариантом '{self.component_config.variant_key}'. "
        ...
        # === ШАГ 2: Обратная совместимость — загрузка из глобального agent_config ===
            ...
            # 2. Загружаем промпты, специфичные для сервиса
            await self._load_service_prompts()

            # 3. Загружаем контракты, специфичные для сервиса
            await self._load_service_contracts()

            self.logger.info(
                f"Сервис '{self.name}' инициализирован. Загружено промптов: {len(self._cached_prompts)}"
            )
            return True
        
        else:
            raise ValueError(
                f"Сервис '{self.name}' не может быть инициализирован: "
    ...
    async def initialize_with_system_config(self, system_resources_config: Any) -> bool:
        ...
        # 2. Загружаем промпты, специфичные для сервиса, из конфигурации системных ресурсов
        await self._load_service_prompts_from_system_config()

        # 3. Загружаем контракты, специфичные для сервиса, из конфигурации системных ресурсов
        await self._load_service_contracts_from_system_config()

        self.logger.info(
            f"Сервис '{self.name}' инициализирован с кэшированием системных ресурсов. Загружено промптов: {len(self._cached_prompts)}"
        )
        return True

    async def _load_service_prompts_from_system_config(self):
        """Загрузка промптов для сервиса из конфигурации системных ресурсов"""
        if hasattr(self._system_resources_config, 'resource_prompt_versions'):
            # Загрузка промптов для сервиса
    ...
    async def _load_service_contracts_from_system_config(self):
        """Загрузка контрактов для сервиса из конфигурации системных ресурсов"""
        if not self.system_context.get_resource("contract_service"):
            return
        
        # Загрузка контрактов для сервиса
    ...
    def get_prompt(self, capability_name: str) -> str:
        ...
        if capability_name not in self._cached_prompts:
            raise RuntimeError(
                f"Промпт для capability '{capability_name}' не загружен в сервис '{self.name}'. "
    ...
    def get_input_contract(self, capability_name: str) -> Dict:
        ...
        if capability_name not in self._cached_input_contracts:
            raise RuntimeError(
                f"Входящий контракт для capability '{capability_name}' не загружен в сервис '{self.name}'. "
    ...
    def get_output_contract(self, capability_name: str) -> Dict:
        ...
        if capability_name not in self._cached_output_contracts:
            raise RuntimeError(
                f"Исходящий контракт для capability '{capability_name}' не загружен в сервис '{self.name}'. "
    ...
    def get_contract(self, contract_name: str) -> Any:
        ...
        raise RuntimeError(
            f"Контракт '{contract_name}' не загружен в сервисе '{self.name}'."
        )
