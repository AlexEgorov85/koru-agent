...
```python
# Remove circular import - this line should not exist
...
from domain.abstractions.tools.file_lister import BaseFileLister
...
class FileListerTool(BaseTool, BaseFileLister):
    ...
    async def list_files(self, directory: str, extensions=None) -> List[str]:
        """
        ...
        """
        # Используем внутреннюю реализацию с параметрами по умолчанию
        input_data = FileListerInput(
            path=directory,
            recursive=False,
            include_files=True,
        ...
        if result.success and result.items:
            # Фильтруем файлы по расширениям если они заданы
            filtered_items = []
            for item in result.items:
                if item.type == "file":
                    if extensions:
                        # Проверяем, соответствует ли расширение файла одному из разрешенных
                        file_ext = os.path.splitext(item.name)[1][1:]  # Получаем расширение без точки
                        if file_ext.lower() in [ext.lower() for ext in extensions]:
                            filtered_items.append(item.path)
                    else:
                        # Если расширения не заданы, добавляем все файлы
                        filtered_items.append(item.path)
