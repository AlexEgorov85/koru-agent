"""
Тесты для модели ExecutionResult (ExecutionResult, ExecutionStatus).
"""
import pytest
...
class TestExecutionResultModel:
    ...
    def test_execution_result_from_dict(self):
        ...
        execution_result = ExecutionResult.model_validate(data)
        
        assert execution_result.status == ExecutionStatus.FAILED
        assert execution_result.result == {"error_data": "something went wrong"}
        assert execution_result.observation_item_id == "obs_error"
        assert execution_result.summary == "Execution failed"
        assert execution_result.error == "Something went wrong"
