...
async def test_prompt_renderer_with_validation():
    ...
    await mock_repo.save_version(version)
...
""",
    ...
    # 5. Проверяем, что снапшот был создан
    assert snapshot_manager.save_snapshot.called, "Должен быть создан снапшот выполнения"
    print("5. Снапшот выполнения успешно создан")
