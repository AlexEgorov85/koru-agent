"""
Расширенное тестирование EventBus и сервисов промтов/контрактов
"""
import sys
import os
...
async def test_event_bus_and_services():
    try:
        ...
        # Проверка EventBus - он доступен как атрибут, а не как ресурс
        print("\n--- Проверка EventBus ---")
        if hasattr(infra, 'event_bus') and infra.event_bus:
            print("[OK] EventBus доступен как атрибут инфраструктурного контекста")
            ...
            # Подписка на событие
            infra.event_bus.subscribe(EventType.SYSTEM_INITIALIZED, test_event_handler)
            print("   Подписка на событие выполнена")
            
            # Публикация события
            await infra.event_bus.publish(
                EventType.SYSTEM_INITIALIZED, 
                data={"test": "event_data", "timestamp": "now", "source": "test"}
            )
            ...
            try:
                ...
                if hasattr(contract_storage, 'contracts'):
                    ...
                    if contract_count > 0:
                        ...
                        print("   [INFO] Нет загруженных контрактов (это нормально для тестовой среды)")
                else:
        ...
        # Проверка прямого доступа к компонентам
        print("\n--- Проверка прямого доступа к компонентам ---")
        print(f"EventBus: {type(infra.event_bus).__name__ if infra.event_bus else 'None'}")
        print(f"PromptStorage: {type(infra.prompt_storage).__name__ if infra.prompt_storage else 'None'}")
        print(f"ContractStorage: {type(infra.contract_storage).__name__ if infra.contract_storage else 'None'}")
        print(f"ResourceRegistry: {type(infra.resource_registry).__name__ if infra.resource_registry else 'None'}")
        print(f"LifecycleManager: {type(infra.lifecycle_manager).__name__ if infra.lifecycle_manager else 'None'}")
        ...
        print("- Прямой доступ к компонентам работает")
